...
SET DROP_DIR=D:\Development\libsbml_out
...
copy %LIBSBML_ROOT%\examples\perl\callExternalValidator.pl callExternalValidator.pl
copy %LIBSBML_ROOT%\examples\perl\convertSBML.pl convertSBML.pl
copy %LIBSBML_ROOT%\examples\perl\createExampleSBML.pl createExampleSBML.pl
...
copy %LIBSBML_ROOT%\examples\layout\convertLayout.cpp convertLayout.cpp
...
copy %LIBSBML_ROOT%\examples\python\readSBML.py readSBML.py
...
cd octave
copy %INSTALL_DIR%\bindings\octave\TranslateSBML.mex TranslateSBML.mex
copy %INSTALL_DIR%\bindings\octave\OutputSBML.mex OutputSBML.mex
...
copy /y "%INSTALL_DIR%\bindings\python\src\dist\libsbml-%VERSION%.win32-py2.5.exe" libSBML-%VERSION%-win-py2.5-x86.exe
copy /y "%INSTALL_DIR%\bindings\python\src\dist\libsbml-%VERSION%.win32-py2.6.exe" libSBML-%VERSION%-win-py2.6-x86.exe
copy /y "%INSTALL_DIR%\bindings\python\src\dist\libsbml-%VERSION%.win32-py2.7.exe" libSBML-%VERSION%-win-py2.7-x86.exe
...
copy "%OLD_DEPENDENCIES_DIR%\bin\bzip2.dll" bzip2.dll
copy "%OLD_DEPENDENCIES_DIR%\bin\iconv.dll" iconv.dll
...
copy "%OLD_DEPENDENCIES_DIR%\lib\bzip2.lib" bzip2.lib
copy "%OLD_DEPENDENCIES_DIR%\lib\iconv.lib" iconv.lib
cd ..
...
echo a | xcopy *.h %INST_BASE_DIR%\libsbml\win32\include\libxml2
 
rem win32/include/bzip2

cd /d %BZIP2_INCLUDE_DIR%
echo a | xcopy *.h %INST_BASE_DIR%\libsbml\win32\include\bzip2

rem win32/include/zlib

cd /d %ZIP_INCLUDE_DIR%
echo a | xcopy *.h %INST_BASE_DIR%\libsbml\win32\include\zlib

rem win32/include/iconv
cd /d %ICONV_INCLUDE_DIR%

echo a | xcopy *.h %INST_BASE_DIR%\libsbml\win32\include\iconv
