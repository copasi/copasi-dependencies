...
%{
	public SBasePlugin DowncastSBasePlugin(long cPtr, boolean owner)
	{
		...
		{
			case (int) libsbml.SBML_FUNCTIONDEFINITION:
	...
	public override SBasePlugin DowncastSBase(long cPtr, boolean owner)
	{
		if (cPtr == 0) return null;
		...
		switch( sb.getTypeCode() )
		{
			...
			case (int) libsbml.SBML_DISTRIB_PREDEFINED_PDF:
				return new PredefinedPDF(cPtr, owner);

			case (int) libsbml.SBML_DISTRIB_EXPLICT_PMF:
				return new ExplicitPMF(cPtr, owner);

			case (int) libsbml.SBML_DISTRIB_EXPLICT_PDF:
				return new ExplicitPDF(cPtr, owner);
...
COVARIANT_RTYPE_CLONE(PredefinedPDF)
COVARIANT_RTYPE_CLONE(ExplicitPMF)
COVARIANT_RTYPE_CLONE(ExplicitPDF)
