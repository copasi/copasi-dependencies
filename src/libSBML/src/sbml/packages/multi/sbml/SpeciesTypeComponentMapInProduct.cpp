                             ...
                             const ExpectedAttributes& expectedAttributes)
{
  ...
  */
  ...
  if (assigned == true)
  {
    ...
    else if (SyntaxChecker::isValidSBMLSId(mReactant) == false)
    {
  ...
  if (assigned == true)
  {
    ...
    else if (SyntaxChecker::isValidSBMLSId(mReactantComponent) == false)
    {
      logError(InvalidIdSyntax);
    }
  }
  ...
  if (assigned == true)
  {
    ...
    else if (SyntaxChecker::isValidSBMLSId(mProductComponent) == false)
    {
      logError(InvalidIdSyntax);
    }
  }
...
ListOfSpeciesTypeComponentMapInProducts::writeXMLNS(XMLOutputStream& stream) const
{
  ...
  if (prefix.empty())
  {
    if (getNamespaces()->hasURI(MultiExtension::getXmlnsL3V1V1()))
    {
